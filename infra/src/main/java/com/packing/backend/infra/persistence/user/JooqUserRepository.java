...
public class JooqUserRepository implements UserRepository {
    ...
    public User save(User user) {
        constraintTranslatorFor(user).translating(() -> writer.upsert(UserRecordMapper.TABLE,
                                                                      UserRecordMapper.toRecord(user),
                                                                      user.version()));
        user.markPersisted();
        return user;
    }
    ...
    private SqlConstraintViolationTranslator constraintTranslatorFor(User user) {
        return new SqlConstraintViolationTranslator(Map.of(
                                                           "uq_users_username",
                                                           () -> new UsernameAlreadyTakenException(user.username()),
                                                           "uq_users_email",
                                                           () -> new EmailAlreadyRegisteredException(user.email()),
